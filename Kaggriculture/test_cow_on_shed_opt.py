"""
Optimized Cow on (4, 4) Engine:
- Cow #1: (4, 4) -> Placed directly on the usable square (4, 4)
- Cow #2: (4, 3) -> 1 step North
- Sheep #1: (3, 4) -> 1 step West
- Sheep #2: (3, 3) -> 1 step Northwest
"""
...
LIVESTOCK_TILES = {
    "NW": {
        (4, 4): ("PASTURE", "COW"),     # Cow #1 placed directly on (4, 4)!
        (4, 3): ("PASTURE", "COW"),     # Cow #2 placed at (4, 3)
        (3, 4): ("PASTURE", "SHEEP"),   # Sheep #1 placed at (3, 4)
        (3, 3): ("PASTURE", "SHEEP"),   # Sheep #2 placed at (3, 3)
    },
    "NE": {},
    "SW": {},
    "SE": {},
...
def get_target_crop_opt(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
...
def agent_cow_on_shed_opt(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_opt(pos, rem_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if w_idx == 0 and day <= 28:
                ...
                # Step 1: If farmer has an animal in shed and no animal in inv, pickup immediately if at (4, 4)
                ...
                # Step 2: If farmer holds an animal, place it (order: (4, 4) Cow 1 first!)
                ...
                # Step 3: If standing at (4, 4) and needs wheat feed, pick up wheat before doing anything else
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_opt(w_pos, rem_days, day)
                    chosen_c = None
