"""
Strategy V3: Master High-Margin 100k+ Industrial Farm Agent.
Combines:
- 100-Tile Land Expansion (NE on Day 0, SW and SE on Day 11)
- High-Margin Portfolio: Melon ($250) + Strawberry ($120) + Wheat ($25)
- Hybrid Livestock: 2 Cows (Milk $160) + 2 Sheep (Wool $200) + 2 Geese (Eggs $50 + Fert $100)
- Aggressive Wave 2 Melon & Strawberry Replanting on 100 tiles (Days 11-25)
- Wave 3 Day 28 Whole-Farm Wheat Blitz (Days 28-30)
- 100% Day 30 Liquidation
"""
import math
...
def get_target_crop_for_pos(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    ...
    # Plot 2 (NE): High Yield Melon & Strawberry Engine
    if x >= 5 and y < 5:
        if day <= 10:
            return "MELON" if (x + y) % 3 != 0 else "STRAWBERRY"
        elif day <= 18 and remaining_days >= 10:
            return "MELON" if (x + y) % 2 == 0 else "STRAWBERRY"
        elif day <= 25:
            return "STRAWBERRY" if (x + y) % 2 == 0 else "WHEAT"
        return "WHEAT"

    # Plot 3 (SW): High Yield Melon & Strawberry Engine
    if x < 5 and y >= 5:
    ...
    # Plot 4 (SE): High Yield Melon & Strawberry Engine
    if x >= 5 and y >= 5:
        if day <= 18 and remaining_days >= 10:
            return "MELON" if (x + y) % 2 == 0 else "STRAWBERRY"
        elif day <= 25:
    ...
    try:
        ...
        if day <= 20 and len(orders) < max_orders:
            if "NE" not in unlocked_quads and cur_money >= 1200 and day <= 5:
                ...
                num_quads += 1
            elif "SE" not in unlocked_quads and cur_money >= 5000 and (day <= 18 or all_plot1_stocked):
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_for_pos(pos, remaining_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_for_pos(w_pos, remaining_days, day)
                    chosen_c = None
