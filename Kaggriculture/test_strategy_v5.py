"""
Strategy V5: Precision Market-Slicing & 100-Tile Land Surge Agent for 100k+.
"""
import math
...
def get_safe_sell_quantity(product: str, current_inv: int, available_in_shed: int, remaining_days: int) -> int:
    ...
    # On the very last day (day 29-30), liquidate everything up to batch limit
    ...
    # For sensitive luxury goods (MELON, MILK, WOOL, STRAWBERRY)
    # Never dump more than 10-15 units per turn so price stays high
    safe_q = 0
...
def get_target_crop_v5(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    if day == 28:
        return "WHEAT"
    ...
    # Plot 2 (NE): Massive Melons on early waves -> Strawberries
    if x >= 5 and y < 5:
        if day <= 19 and remaining_days >= 9:
            if (x + y) % 4 == 0:
                return "STRAWBERRY" if day <= 12 else "WHEAT"
            return "MELON"
        return "WHEAT"

    # Plot 3 (SW): Melons & Strawberries
    if x < 5 and y >= 5:
        if day <= 19 and remaining_days >= 9:
            if (x + y) % 3 == 0:
                return "STRAWBERRY"
...
def agent_v5(obs: dict) -> dict:
    try:
        ...
        # Livestock Queue (2 Cows + 2 Sheep) - Buy starting on Day 1+ when wheat feed is ready
        target_cows = 2
        target_sheep = 2
        ...
        # NE unlocked early with capital protection
        # SW and SE unlocked as soon as Day 10-12 Melon wave provides $8,000+ cash!
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_v5(pos, remaining_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_v5(w_pos, remaining_days, day)
                    chosen_c = None
