"""
Test Cow on Shed (4, 4) Layout:
- 2 Cows + 2 Sheep:
  - (4, 4): COW (directly on Shed tile!)
  - (4, 3): COW (1 step North)
  - (3, 4): SHEEP (1 step West)
  - (3, 3): SHEEP (1 step Northwest)
"""
...
# 4 Livestock Plots with Cow #1 directly ON Shed (4, 4)
LIVESTOCK_TILES = {
    "NW": {
        (4, 4): ("PASTURE", "COW"),     # Cow #1 directly on Shed (4, 4)!
        (4, 3): ("PASTURE", "COW"),     # Cow #2: 1 step North of Shed
        (3, 4): ("PASTURE", "SHEEP"),   # Sheep #1: 1 step West of Shed
        (3, 3): ("PASTURE", "SHEEP"),   # Sheep #2: 1 step Northwest of Shed
    },
    "NE": {},
    "SW": {},
    "SE": {},
...
# Dedicated Wheat Coordinates in NW
...
def get_target_crop_cow_shed(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
...
def agent_cow_on_shed(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        # 2. LIVESTOCK PROCUREMENT (2 Cows + 2 Sheep: First Cow goes on Shed (4, 4)!)
        target_cows = 2
        target_sheep = 2
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_cow_shed(pos, rem_days, day)
        ...
        # 5. SHED INVENTORY LIQUIDATION (Reserve 8 Wheat for feed & 4 Fertilizer)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if w_idx == 0 and day <= 28:
                ...
                # If farmer has no animal in inv, check if shed has one:
                if not has_animal_in_inv and has_animal_in_shed:
                    # If already standing at (4, 4), pick it up immediately!
                    if w_pos == (4, 4):
                ...
                # If farmer carries an animal in inv:
                elif has_animal_in_inv:
                    # Find first target pasture that needs placement (order: (4,4) Cow first!)
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_cow_shed(w_pos, rem_days, day)
                    chosen_c = None
