"""
Test 12-Livestock Mega Hub: Plot 1 (NW) + Plot 2 (NE)
Plot 1 (NW): 4 Cows + 2 Sheep
- Cows: (4, 4), (4, 3), (4, 2), (2, 4)
- Sheep: (3, 4), (3, 3)

Plot 2 (NE): 4 Cows + 2 Sheep
- Cows: (5, 4), (5, 3), (5, 2), (7, 4)
- Sheep: (6, 4), (6, 3)
"""
...
# Dedicated Wheat Coordinates in NW and NE to sustain all 12 animals
...
def get_target_crop_12a(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
...
def agent_12animals(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        # Total Targets: 8 Cows + 4 Sheep
        target_cows = 4 if "NE" not in state.unlocked_quadrants else 8
        target_sheep = 2 if "NE" not in state.unlocked_quadrants else 4

        # Count active + held animals
        all_livestock_plots = {}
        for q in ["NW", "NE"]:
            all_livestock_plots.update(LIVESTOCK_TILES.get(q, {}))

        total_cows = sum(1 for p, info in all_livestock_plots.items() if info[1] == "COW" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "COW") + state.shed.get("COW", 0) + sum(inv.get("COW", 0) for inv in state.inventories)
        total_sheep = sum(1 for p, info in all_livestock_plots.items() if info[1] == "SHEEP" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "SHEEP") + state.shed.get("SHEEP", 0) + sum(inv.get("SHEEP", 0) for inv in state.inventories)
        ...
        plot1_stocked = (total_cows >= 4 and total_sheep >= 2)
        plot2_stocked = (total_cows >= 8 and total_sheep >= 4)

        # 3. CONTROLLED LAND EXPANSION
        if day <= 20 and len(orders) < max_orders:
            if "NE" not in state.unlocked_quadrants and money >= 2500 and day <= 10 and plot1_stocked:
                orders.append(["BUY_LAND", "NE"])
                money -= 1000
                num_quads += 1
            elif "SW" not in state.unlocked_quadrants and money >= 5000 and day <= 16 and plot2_stocked:
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_12a(pos, rem_days, day)
        ...
        # Placement sequence: Plot 1 then Plot 2
        placement_order = [
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if w_idx == 0 and day <= 28:
                ...
                # Step 1: If farmer has an animal in shed and no animal in inv, pickup immediately if at shed access
                ...
                # Step 2: If farmer holds an animal, place it
                ...
                # Step 3: If standing at shed access and needs wheat feed, pick up wheat
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_12a(w_pos, rem_days, day)
                    chosen_c = None
