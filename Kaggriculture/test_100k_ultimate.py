"""
100k Ultimate Engine:
- Plot 1 (NW): 4-Livestock Hub (Cow on (4,4), Cow on (4,3), Sheep on (3,4), Sheep on (3,3)) + 7 Wheat + Strawberries/Melons
- Plot 2 (NE): High Yield Melons + Tomatoes (absorbed by Pizza Shop & Farmers Market at $280/unit)
- Plot 3 (SW): High Yield Melons + Carrots (absorbed by Pet Cafe & Farmers Market at $116/unit)
- Plot 4 (SE): Fast Wheat & Carrots
- Smart Fertilization: Uses daily livestock fertilizer on Melons and Tomatoes during bonus windows
- Market Liquidation: Synchronized with town shop drain ticks (step % 4 == 0)
"""
...
def get_target_crop_ultimate(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    if day == 28: return "WHEAT"
    if day >= 29 or (day >= 26 and day != 28) or remaining_days <= 1: return None

    x, y = pos
    # Plot 1 (NW): Wheat + Melons (Days 0-10) + Ongoing Strawberries (Days 10-25)
    if x < 5 and y < 5:
    ...
    # Plot 2 (NE): High Yield Melons + Tomatoes (Tomatoes absorbed by Pizza Shop & Farmers Market at $280!)
    if x >= 5 and y < 5:
        if day <= 19 and remaining_days >= 10:
            if (x + y) % 4 == 0: return "TOMATO"
            elif (x + y) % 4 != 1: return "MELON"
            return "WHEAT"
        return "WHEAT"

    # Plot 3 (SW): High Yield Melons + Carrots (Carrots absorbed by Pet Cafe & Farmers Market at $116!)
    if x < 5 and y >= 5:
        if day <= 15 and remaining_days >= 10:
            if (x + y) % 3 == 0: return "MELON"
            elif (x + y) % 3 == 1: return "CARROT"
            return "WHEAT"
        elif day <= 25 and remaining_days >= 4:
            return "CARROT"
        return "WHEAT"

    # Plot 4 (SE): High Yield Melons & Fast Carrots
    if x >= 5 and y >= 5:
        if day <= 16 and remaining_days >= 12: return "MELON"
        elif day <= 25 and remaining_days >= 4: return "CARROT"
        return "WHEAT"

    return "WHEAT"


def agent_ultimate(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        base_hires = 5 if num_quads == 1 else (7 if num_quads == 2 else (9 if num_quads == 3 else 11))
        harv_count = len(state.get_harvestable_tiles())

        if harv_count >= 15 and money >= 1000: base_hires = min(base_hires + 1, 11)
        if harv_count >= 25 and money >= 3000: base_hires = min(base_hires + 1, 12)
        ...
        # 2. LIVESTOCK PROCUREMENT (2 Cows + 2 Sheep: First Cow on (4, 4)!)
        target_cows = 2
        target_sheep = 2
        total_cows = sum(1 for p, info in LIVESTOCK_TILES["NW"].items() if info[1] == "COW" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "COW") + state.shed.get("COW", 0) + sum(inv.get("COW", 0) for inv in state.inventories)
        total_sheep = sum(1 for p, info in LIVESTOCK_TILES["NW"].items() if info[1] == "SHEEP" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "SHEEP") + state.shed.get("SHEEP", 0) + sum(inv.get("SHEEP", 0) for inv in state.inventories)
        ...
        all_plot1_fully_stocked = (total_cows >= target_cows and total_sheep >= target_sheep)

        # 3. CONTROLLED LAND EXPANSION
        if day <= 20 and len(orders) < max_orders and all_plot1_fully_stocked:
            if "NE" not in state.unlocked_quadrants and money >= 2200 and day <= 10:
                orders.append(["BUY_LAND", "NE"])
                money -= 1000
                num_quads += 1
            elif "SW" not in state.unlocked_quadrants and money >= 4500 and day <= 15:
                orders.append(["BUY_LAND", "SW"])
                money -= 2000
                num_quads += 1
            elif "SE" not in state.unlocked_quadrants and money >= 7500 and day <= 18:
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_ultimate(pos, rem_days, day)
            ...
            for crop in ["WHEAT", "MELON", "TOMATO", "CARROT", "STRAWBERRY"]:
        ...
        placement_order = [(4, 4), (4, 3), (3, 4), (3, 3)]
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_ultimate(w_pos, rem_days, day)
                    chosen_c = None
...
if __name__ == "__main__":
    ...
    env.run([agent_ultimate, "starter"])
    print(f"\n=======================================================")
    print(f"ULTIMATE SCORE: ${env.steps[-1][0].reward:,.2f}")
    print(f"=======================================================\n")
