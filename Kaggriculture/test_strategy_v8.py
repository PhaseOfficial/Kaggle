"""
Strategy V8: Dynamic Zero-Wither Dispatcher & Global Workforce Coordination.
Key Fixes:
1. Global Urgent Water Override: No crop ever misses a 2nd day of water (Zero withering).
2. Overflow Work Assisting: When a worker's assigned quadrant has no pending tasks, they immediately cross-assist neighboring quadrants.
3. Weed Extinction: Clear weeds immediately with highest priority before they spread.
4. Smooth Sliced Selling on every turn.
"""
import math
...
def get_target_crop_v8(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    if day == 28:
        return "WHEAT"
    ...
    # Plot 3 (SW): High-Yield Melon & Wheat
    if x < 5 and y >= 5:
        if day <= 19 and remaining_days >= 10:
            if (x + y) % 3 == 0:
                return "MELON"
    ...
    # Plot 4 (SE): High-Yield Melon & Wheat
    if x >= 5 and y >= 5:
        if day <= 19 and remaining_days >= 10:
            if (x + y) % 3 == 0:
                return "MELON"
...
def agent_v8(obs: dict) -> dict:
    try:
        ...
        # Animals on Day 2+ (once wheat feed is guaranteed in shed)
        target_cows = 2
        target_sheep = 2
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_v8(pos, remaining_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_v8(w_pos, remaining_days, day)
                    chosen_c = None
            ...
            # --- D. BFS Navigation (With Dynamic Global Overflow Assisting) ---
            if action is None:
                best_target = None
                ...
                # Priority 0: Livestock needs for NW workers
                ...
                # Priority 1: URGENT WATER (GLOBAL OVERRIDE - NEVER LET A PLANT WITHER!)
                if not best_target and day <= 29 and urgent_water:
                ...
                # Priority 4: Dig Weeds (Global & Local)
                ...
                # Priority 5: Empty Planting (Local -> Global)
                ...
                # Priority 6: Routine Water (Local -> Global Cross-Assist!)
                if not best_target and day <= 29 and routine_water:
