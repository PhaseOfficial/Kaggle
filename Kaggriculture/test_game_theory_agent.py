"""
Game-Theoretic & Markov Perfect Decision Engine for Kaggriculture.

Mathematical Pillars:
1. Cournot Competition & Opponent Forecasting:
   - Tracks opponent crop portfolios and harvest maturation days.
   - Front-runs market gluts and diversifies when opponent over-supplies melons.
2. Markov Price Peak Arbitrage:
   - Slices sales synchronized with the 4-turn town shop consumption cycle (hour % 4 == 1).
   - Holds products when temporarily depressed and liquidates during shop-driven scarcity.
3. Bellman Marginal-Value Worker Dispatching:
   - Evaluates actions by expected Markov future value Q(s, a).
   - Dynamic Fibonacci workforce scaling.
"""
...
def get_safe_sell_quantity(
    ...
    opp_heavy_on_product: bool = False,
) -> int:
    ...
    if remaining_days <= 2:
        return min(available_in_shed, 25)

    if product in ("WHEAT", "EGG"):
        return min(available_in_shed, 20)

    # Dynamic pricing threshold: if opponent is flooding, demand a higher price floor or hold
    min_ratio = 0.55 if opp_heavy_on_product else 0.45
    min_price = max(PRICE_FLOOR, int(base * min_ratio))

    safe_q = 0
    test_inv = current_inv
    # Max batch per turn to prevent quadratic price depression
    batch_limit = 12 if (product == "MELON") else 15
    for _ in range(min(available_in_shed, batch_limit)):
...
class FarmState:
    def __init__(self, obs: dict):
        ...
        self.opp_farm = farms[self.opp_id] if self.opp_id < len(farms) else {}
        ...
        # Opponent State Analysis (Cournot Market Forecast)
        self.opp_crop_counts = Counter()
        self.opp_harvest_forecast = Counter()
        for row in self.opp_farm.get("tiles", []):
            for t in row:
                if isinstance(t, dict) and t.get("kind") == "PLANT":
                    crop = t.get("crop")
                    self.opp_crop_counts[crop] += 1
                    p_day = t.get("planted_day", 0)
                    c_info = CROPS.get(crop, {})
                    self.opp_harvest_forecast[p_day + c_info.get("max_yield_day", 4)] += 1

        self.opp_heavy_melons = self.opp_crop_counts.get("MELON", 0) >= 12
...
def get_target_crop_gt(pos: tuple[int, int], remaining_days: int, day: int, opp_heavy_melons: bool) -> str | None:
    ...
    if x < 5 and y >= 5:
        if day <= 15 and remaining_days >= 10:
            if (x + y) % 3 == 0 and not opp_heavy_melons: return "MELON"
            return "WHEAT"
        return "WHEAT"

    # Plot 4 (SE)
    if x >= 5 and y >= 5:
        if day <= 15 and remaining_days >= 10 and (x + y) % 3 == 0 and not opp_heavy_melons:
            return "MELON"
        return "WHEAT"
...
def gt_agent(obs: dict) -> dict:
    try:
        ...
        target_cows = 2
        target_sheep = 2
        total_cows = sum(1 for p, info in LIVESTOCK_PLOTS["NW"].items() if info[1] == "COW" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "COW") + state.shed.get("COW", 0) + sum(inv.get("COW", 0) for inv in state.inventories)
        total_sheep = sum(1 for p, info in LIVESTOCK_PLOTS["NW"].items() if info[1] == "SHEEP" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "SHEEP") + state.shed.get("SHEEP", 0) + sum(inv.get("SHEEP", 0) for inv in state.inventories)
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_gt(pos, rem_days, day, state.opp_heavy_melons)
        ...
        # 5. MARKOV ARBITRAGE LIQUIDATION
        for product in PRODUCTS:
            ...
            safe_qty = get_safe_sell_quantity(
                ...
                opp_heavy_on_product=(state.opp_heavy_melons if product == "MELON" else False),
            )
            if safe_qty > 0:
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_gt(w_pos, rem_days, day, state.opp_heavy_melons)
                    chosen_c = None
