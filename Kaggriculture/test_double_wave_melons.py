"""
Double-Wave Mega-Melon Strategy:
- Wave 1 Melons (Days 0-12) across NW + NE
- Wave 2 Melons (Days 12-24) across NW + NE + SW + SE
- Wave 3 Fast Carrots/Wheat (Days 24-28)
- 4 Cows + 2 Sheep running continuously from Day 2 in Plot 1
"""
...
def get_target_crop_double_wave(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    ...
    if x < 5 and y < 5:
        ...
        elif day <= 17 and remaining_days >= 12: return "MELON"
        elif day <= 24: return "CARROT"
        return "WHEAT"

    # Plot 2 (NE)
    if x >= 5 and y < 5:
        if day <= 18 and remaining_days >= 12: return "MELON"
        elif day <= 24: return "CARROT"
        return "WHEAT"
...
def agent_double_wave(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        # 2. LIVESTOCK PROCUREMENT (4 Cows + 2 Sheep in NW Shed Hub)
        target_cows = 4
        target_sheep = 2
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_double_wave(pos, rem_days, day)
            ...
            for crop in ["WHEAT", "MELON", "CARROT", "STRAWBERRY"]:
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_double_wave(w_pos, rem_days, day)
                    chosen_c = None
...
if __name__ == "__main__":
    ...
    env.run([agent_double_wave, "starter"])
    print(f"\n=======================================================")
    print(f"DOUBLE WAVE SCORE: ${env.steps[-1][0].reward:,.2f}")
    print(f"=======================================================\n")
