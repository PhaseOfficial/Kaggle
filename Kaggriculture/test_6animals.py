"""
Test 6 Livestock Setup: 4 Cows + 2 Sheep tightly hugging Shed (4, 4) in NW.
Unlocks Quadrant 2 (NE) ONLY after all 4 Cows and 2 Sheep are fully purchased and active.
"""
...
# 6 Livestock Plots tightly hugging Shed (4, 4) in NW: 4 Cows + 2 Sheep
LIVESTOCK_TILES = {
    "NW": {
        (4, 3): ("PASTURE", "COW"),     # 1 step North of Shed (4, 4)
        (3, 4): ("PASTURE", "COW"),     # 1 step West of Shed (4, 4)
        (4, 2): ("PASTURE", "COW"),     # 2 steps North of Shed (4, 4)
        (3, 3): ("PASTURE", "COW"),     # 1 step Northwest of Shed (4, 4)
        (2, 4): ("PASTURE", "SHEEP"),   # 2 steps West of Shed (4, 4)
        (3, 2): ("PASTURE", "SHEEP"),   # Northwest step (3, 2)
    },
    "NE": {},
    "SW": {},
    "SE": {},
...
# 8 Dedicated Wheat Coordinates in NW to sustain 6 animals
...
def get_target_crop_6a(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
...
def agent_6animals(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        # 2. LIVESTOCK PROCUREMENT (4 Cows -> 2 Sheep strictly on Plot 1 NW)
        target_cows = 4
        target_sheep = 2
        ...
        # Strict Requirement: All 4 Cows and 2 Sheep MUST be acquired BEFORE expanding to NE (Quadrant 2)
        all_plot1_fully_stocked = (total_cows >= target_cows and total_sheep >= target_sheep)

        # 3. CONTROLLED LAND EXPANSION (Only after all 4 Cows and 2 Sheep are stocked!)
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_6a(pos, rem_days, day)
        ...
        # 5. SHED INVENTORY LIQUIDATION (Reserve 10 Wheat for feed & 4 Fertilizer)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_6a(w_pos, rem_days, day)
                    chosen_c = None
