"""
Comprehensive Master Industrial Agriculture Agent (Target: 100,000+).
"""
import math
...
# 6 Livestock Hub Plots directly around Shed (4, 4) in Plot 1 (NW)
LIVESTOCK_PLOTS = {
    (4, 3): ("PASTURE", "COW"),
    (3, 4): ("PASTURE", "COW"),
    (3, 3): ("PASTURE", "SHEEP"),
...
NW_WHEAT_TILES = {(1, 4), (0, 4), (4, 1), (4, 0), (1, 3), (3, 2), (0, 3)}
...
def get_safe_sell_quantity(product: str, current_inv: int, available_in_shed: int, remaining_days: int) -> int:
    ...
    # Scarcity premium check: if inventory is below I0, sell more aggressively to capture top-dollar prices!
    if current_inv < MARKET_I0:
...
def get_target_crop_master(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    if day == 28:
        return "WHEAT"
    ...
    x, y = pos
    # Plot 1 (NW): Dedicated wheat for feed + Early Melons
    if x < 5 and y < 5:
    ...
    # Plot 2 (NE): High Yield Melon & Strawberry Engine (Days 0-19)
    if x >= 5 and y < 5:
        if day <= 19 and remaining_days >= 10:
            if (x + y) % 4 != 0:
                return "MELON"
            return "STRAWBERRY" if day <= 10 else "WHEAT"
        return "WHEAT"

    # Plot 3 (SW): Melons & Wheat
    if x < 5 and y >= 5:
        if day <= 16 and remaining_days >= 10 and (x + y) % 3 == 0:
            return "MELON"
        return "WHEAT"

    # Plot 4 (SE): High Turnover Wheat & Strategic Melons
    if x >= 5 and y >= 5:
        if day <= 16 and remaining_days >= 10 and (x + y) % 3 == 0:
            return "MELON"
        return "WHEAT"
...
def master_agent(obs: dict) -> dict:
    try:
        ...
        # Workforce: 5 -> 7 -> 9 -> 12
        target_hires = 5 if num_quads == 1 else (7 if num_quads == 2 else (9 if num_quads == 3 else 12))
        ...
        # Livestock Queue: 2 Cows + 2 Sheep + 2 Geese
        target_cows = 2
        target_sheep = 2
        target_geese = 2
        ...
        total_geese = sum(1 for p, info in LIVESTOCK_PLOTS.items() if info[1] == "GOOSE" and isinstance(get_tile(*p), dict) and get_tile(*p).get("animal") == "GOOSE") + shed.get("GOOSE", 0) + sum(inv.get("GOOSE", 0) for inv in inventories)

        if (day >= 1 or shed.get("WHEAT", 0) >= 3) and day <= 20 and len(orders) < max_orders:
        ...
        # FULL 100-TILE EXPANSION (NE -> SW -> SE)
        if day <= 20 and len(orders) < max_orders and all_plot1_stocked:
            if "NE" not in unlocked_quads and cur_money >= 2500 and day <= 8:
                ...
                num_quads += 1
            elif "SW" not in unlocked_quads and cur_money >= 4500 and day <= 15:
                orders.append(["BUY_LAND", "SW"])
                cur_money -= 2000
                unlocked_quads.add("SW")
                num_quads += 1
            elif "SE" not in unlocked_quads and cur_money >= 5500 and day <= 18:
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_master(pos, remaining_days, day)
        ...
        # Shed Liquidation: Sliced selling prevents price crashes
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if w_idx == 0 and day <= 28:
                has_anim_inv = any(inv_w.get(a, 0) > 0 for a in ["COW", "SHEEP", "GOOSE"])
                has_anim_shed = any(shed.get(a, 0) > 0 for a in ["COW", "SHEEP", "GOOSE"])
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_master(w_pos, remaining_days, day)
                    chosen_c = None
