"""
Fluid Shed Liquidation & Zero Waste Master Agent to reach $100k+.
Key fixes:
1. Shed capacity guard: Keep shed items under 35 so end-of-day drop NEVER discards goods.
2. Rapid animal placement on Days 1-5 so all 8 cows + 4 sheep produce for 25+ days.
3. Exploit high market prices ($280 Tomato, $280 Milk, $250 Melon, $224 Strawberry, $116 Carrot).
4. Full workforce scaling to water and fertilize every single melon and ongoing crop daily.
"""
...
class FarmState:
    ...
    def get_empty_tiles(self) -> list[tuple[int, int]]:
        livestock_plots = set()
        for q in self.unlocked_quadrants:
            livestock_plots.update(LIVESTOCK_TILES.get(q, {}).keys())
...
def get_target_crop_mega(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    ...
    if x >= 5 and y < 5:
        ...
        if day <= 8: return "MELON"
        elif day <= 16: return "STRAWBERRY"
        elif day <= 18: return "MELON"
        return "WHEAT"

    # Plot 3 (SW): Mega Melon & Tomato Production
    if x < 5 and y >= 5:
...
def agent_shed_fluid(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        has_ne = ("NE" in state.unlocked_quadrants)
        target_cows = 8 if has_ne else 4
        target_sheep = 4 if has_ne else 2

        active_p1_cows = sum(1 for p, info in LIVESTOCK_TILES["NW"].items() if info[1] == "COW" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "COW")
        active_p1_sheep = sum(1 for p, info in LIVESTOCK_TILES["NW"].items() if info[1] == "SHEEP" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "SHEEP")

        active_p2_cows = sum(1 for p, info in LIVESTOCK_TILES["NE"].items() if info[1] == "COW" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "COW")
        active_p2_sheep = sum(1 for p, info in LIVESTOCK_TILES["NE"].items() if info[1] == "SHEEP" and isinstance(state.get_tile(*p), dict) and state.get_tile(*p).get("animal") == "SHEEP")

        total_cows_held = active_p1_cows + active_p2_cows + state.shed.get("COW", 0) + sum(inv.get("COW", 0) for inv in state.inventories)
        total_sheep_held = active_p1_sheep + active_p2_sheep + state.shed.get("SHEEP", 0) + sum(inv.get("SHEEP", 0) for inv in state.inventories)

        # Only buy an animal if shed has room and we have placed previous ones!
        shed_occupied = sum(state.shed.values())
        if (day >= 1 or state.shed.get("WHEAT", 0) >= 2) and day <= 22 and len(orders) < max_orders and shed_occupied < 30:
        ...
        if day <= 20 and len(orders) < max_orders:
            if "NE" not in state.unlocked_quadrants and money >= 1500 and day <= 14 and plot1_stocked:
                orders.append(["BUY_LAND", "NE"])
                money -= 1000
                num_quads += 1
            elif "SW" not in state.unlocked_quadrants and "NE" in state.unlocked_quadrants and all_12_animals_procured and money >= 2600 and day <= 17:
                orders.append(["BUY_LAND", "SW"])
                money -= 2000
                num_quads += 1
            elif "SE" not in state.unlocked_quadrants and "SW" in state.unlocked_quadrants and money >= 5000 and day <= 18:
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_mega(pos, rem_days, day)
                    if c: needed_counts[c] += 1

            wheat_buffer = max(0, 16 - state.seeds.get("WHEAT", 0))
        ...
        # 5. FLUID SHED LIQUIDATION (Zero-waste: Keep shed room open so midnight drop never discards!)
        # Sell high-value products when available, sell in batches of 1-3 to maximize shop absorption
        ...
        for product in sell_priority:
            ...
            if product == "WHEAT" and day < 29:
                if qty > 16: orders.append(["SELL", "WHEAT", min(qty - 16, 10)])
                continue
            ...
            # Sell fluidly 1-3 units every turn or more if shed is getting full
            batch = min(qty, 3) if shed_occupied < 40 else min(qty, 10)
        ...
        for q in state.unlocked_quadrants:
            active_livestock.update(LIVESTOCK_TILES.get(q, {}))
        ...
        placement_order = [
            (4, 4), (4, 3), (4, 2), (2, 4), (3, 4), (3, 3),
            (5, 4), (5, 3), (5, 2), (7, 4), (6, 4), (6, 3),
        ]

        shed_access_tiles = {(4, 4), (5, 4)} if "NE" in state.unlocked_quadrants else {(4, 4)}
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if w_idx == 0 and day <= 28:
                ...
                if not has_animal_in_inv and has_animal_in_shed:
                    if w_pos in shed_access_tiles:
                        ...
                        closest_shed = min(shed_access_tiles, key=lambda p: abs(p[0] - w_pos[0]) + abs(p[1] - w_pos[1]))
                        step = get_bfs_step(w_pos, closest_shed, unlocked_set)
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_mega(w_pos, rem_days, day)
                    chosen_c = None
                    if pref_c and available_seeds.get(pref_c, 0) > 0:
                        ...
                        chosen_c = "STRAWBERRY"
                    elif available_seeds.get("TOMATO", 0) > 0:
                        chosen_c = "TOMATO"
...
if __name__ == "__main__":
    ...
    env.run([agent_shed_fluid, "starter"])
    print(f"Fluid Agent Score: ${env.steps[-1][0].reward:,.2f}")
