"""
Multi-Commodity Portfolio Diversification Engine to achieve $100k+.
Maintains 7 simultaneous revenue streams: Milk, Wool, Fertilizer, Melon, Strawberry, Tomato, Carrot.
Sells dynamically to the highest-priced unsaturated commodity market.
"""
...
def get_target_crop_portfolio(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    ...
    # Plot 2 (NE): Melons & Tomatoes
    if x >= 5 and y < 5:
        if day <= 6: return "TOMATO"
        elif day <= 18: return "MELON"
        elif day <= 24: return "CARROT"
        return "WHEAT"

    # Plot 3 (SW): Strawberries & Melons
    if x < 5 and y >= 5:
        if day <= 8: return "STRAWBERRY"
        elif day <= 18: return "MELON"
        elif day <= 24: return "CARROT"
        return "WHEAT"

    # Plot 4 (SE): Melons & Carrots
    if x >= 5 and y >= 5:
...
def agent_portfolio(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_portfolio(pos, rem_days, day)
        ...
        # 5. DYNAMIC PRICE-MAXIMIZING SHED LIQUIDATION
        # Sort products by current market price descending!
        available_products = [p for p in PRODUCTS if state.shed.get(p, 0) > 0]
        sorted_by_price = sorted(available_products, key=lambda p: -state.market_prices.get(p, 0))
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_portfolio(w_pos, rem_days, day)
                    chosen_c = None
...
if __name__ == "__main__":
    ...
    env.run([agent_portfolio, "starter"])
    print(f"\n=======================================================")
    print(f"MULTI-COMMODITY PORTFOLIO SCORE: ${env.steps[-1][0].reward:,.2f}")
    print(f"=======================================================\n")
