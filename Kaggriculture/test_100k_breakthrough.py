"""
100k Breakthrough Engine:
- Plot 1 (NW): 4 Cows ((4,4), (4,3), (4,2), (2,4)) + 2 Sheep ((3,4), (3,3)) + 8 Dedicated Wheat
- Plot 2 (NE): High Yield Melon & Strawberry Wave (Days 7-19)
- Plot 3 (SW): High Yield Melon & Carrot Grid (Days 12-24)
- Plot 4 (SE): High Yield Melon & Speed Carrot Terminal Wave (Days 16-28)
- Dynamic town shop price-maximizer
"""
...
def get_target_crop_breakthrough(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
...
def agent_breakthrough(obs: dict) -> dict:
    try:
        state = FarmState(obs)
        orders = []
        max_orders = 10
        ...
        # 2. LIVESTOCK PROCUREMENT (4 Cows + 2 Sheep on NW Shed Hub)
        target_cows = 4
        target_sheep = 2
        ...
        if day <= 20 and len(orders) < max_orders and all_plot1_fully_stocked:
            if "NE" not in state.unlocked_quadrants and money >= 2500 and day <= 10:
                ...
                money -= 2000
                num_quads += 1
            elif "SE" not in state.unlocked_quadrants and money >= 8000 and day <= 18:
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_breakthrough(pos, rem_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_breakthrough(w_pos, rem_days, day)
                    chosen_c = None
...
if __name__ == "__main__":
    ...
    env.run([agent_breakthrough, "starter"])
    print(f"\n=======================================================")
    print(f"BREAKTHROUGH SCORE: ${env.steps[-1][0].reward:,.2f}")
    print(f"=======================================================\n")
