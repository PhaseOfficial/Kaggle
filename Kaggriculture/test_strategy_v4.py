"""
Strategy V4: 3-Wave Pure High-Yield Engine for 100k+.
Rules:
1. Land Expansion:
   - Day 0: Buy NE ($1,000)
   - Day 10 (Melon harvest): Buy SW ($2,000) & SE ($4,000) -> 100 tiles active!
2. Crop Waves (Synchronized):
   - Wave 1 (Days 0-10): Melons (Plots 1 & 2) + Wheat (Plot 1 feed) + Strawberries (Plot 2)
   - Wave 2 (Days 10-20): Replant all 100 tiles with Melons (70%) + Strawberries (20%) + Wheat (10%)
   - Wave 3 (Days 20-28): Replant Melons (Day 20->28 fertilized) + Wheat (Plot 1)
   - Wave 4 (Days 28-30): Day 28 Wheat Blitz (100% empty tiles) -> Day 30 Grand Harvest & Liquidation
3. High-Priority Weed Clearance:
   - Weeds cleared immediately on sight so 100% of tiles are active.
4. Livestock Protection:
   - Always keep 10 wheat buffer in shed so cows/sheep never starve.
"""
import math
...
def get_safe_sell_quantity(product: str, current_inv: int, available_in_shed: int, remaining_days: int) -> int:
    ...
    min_acceptable = 0.35 if product == "MELON" else 0.45
...
def get_target_crop_v4(pos: tuple[int, int], remaining_days: int, day: int) -> str | None:
    # Final Blitz
    if day == 28:
        return "WHEAT"
    ...
    x, y = pos
    # Plot 1 (NW): Dedicated wheat for feed + Melons on early wave
    if x < 5 and y < 5:
    ...
    # Plot 2 (NE): Pure Melons & Strawberries
    if x >= 5 and y < 5:
        if day <= 10:
            return "MELON" if (x + y) % 4 != 0 else "STRAWBERRY"
        elif day <= 20 and remaining_days >= 8:
            return "MELON" if (x + y) % 3 != 0 else "STRAWBERRY"
        return "WHEAT"

    # Plot 3 (SW): Pure Melons & Strawberries
    if x < 5 and y >= 5:
        if day <= 20 and remaining_days >= 8:
            return "MELON" if (x + y) % 4 != 0 else "STRAWBERRY"
        return "WHEAT"

    # Plot 4 (SE): Pure Melons & Strawberries
    if x >= 5 and y >= 5:
        if day <= 20 and remaining_days >= 8:
            return "MELON" if (x + y) % 4 != 0 else "STRAWBERRY"
        return "WHEAT"
...
def agent_v4(obs: dict) -> dict:
    try:
        ...
        # Workforce: scales with land (5 -> 7 -> 10 -> 13)
        ...
        # Livestock Queue: 2 Cows + 2 Sheep (strictly once wheat feed is available!)
        target_cows = 2
        target_sheep = 2
        ...
        has_feed = (day >= 4 or shed.get("WHEAT", 0) >= 3)
        ...
        if day <= 20 and len(orders) < max_orders:
            if "NE" not in unlocked_quads and cur_money >= 1200 and day <= 5:
                ...
                num_quads += 1
            elif "SW" not in unlocked_quads and cur_money >= 3500 and (day <= 15 or all_plot1_stocked):
                orders.append(["BUY_LAND", "SW"])
                cur_money -= 2000
                unlocked_quads.add("SW")
                num_quads += 1
            elif "SE" not in unlocked_quads and cur_money >= 4500 and (day <= 18 or all_plot1_stocked):
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_v4(pos, remaining_days, day)
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_v4(w_pos, remaining_days, day)
                    chosen_c = None
            ...
            if action is None:
                ...
                # High Priority: Dig Weeds so field never gets blocked
