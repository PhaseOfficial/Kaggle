"""
Strategy V2: 100k+ Full Farm Industrial Agriculture Agent.
"""
import math
...
# 6 Goose Coops in NW around Shed (4, 4)
LIVESTOCK_TILES = {
    (4, 3): ("COOP", "GOOSE"),
    (3, 4): ("COOP", "GOOSE"),
    (3, 3): ("COOP", "GOOSE"),
...
NW_WHEAT_TILES = {(1, 4), (0, 4), (4, 1), (4, 0), (3, 1), (3, 0), (2, 2), (1, 3), (0, 3)}
...
def get_safe_sell_quantity(product: str, current_inv: int, available_in_shed: int, remaining_days: int) -> int:
    ...
    if product == "FERTILIZER": return min(available_in_shed, 10)
    safe_q = 0
...
def get_target_crop_pos(pos: tuple[int, int], day: int, remaining_days: int) -> str | None:
    if day == 28:
        return "WHEAT"
    ...
    # Plot 2 (NE)
    if x >= 5 and y < 5:
        if day <= 18 and remaining_days >= 10:
            if (x + y) % 4 == 0:
                return "STRAWBERRY" if day <= 8 else "CARROT"
            return "MELON"
        elif day <= 25:
...
def agent_v2(obs: dict) -> dict:
    try:
        ...
        # Workforce
        target_hires = 5 if num_quads == 1 else (7 if num_quads == 2 else (9 if num_quads == 3 else 11))
        ...
        # Land Expansion: NE -> SW -> SE
        # Buy NE early (Day 0-5) if money >= 1200
        # Buy SW when money >= 3000
        # Buy SE when money >= 5000
        if len(orders) < max_orders and day <= 20:
            if "NE" not in unlocked_quads and cur_money >= 1200 and day <= 5:
                orders.append(["BUY_LAND", "NE"])
                cur_money -= 1000
                unlocked_quads.add("NE")
                num_quads += 1
            elif "SW" not in unlocked_quads and cur_money >= 3000 and day <= 12:
                orders.append(["BUY_LAND", "SW"])
                cur_money -= 2000
                unlocked_quads.add("SW")
                num_quads += 1
            elif "SE" not in unlocked_quads and cur_money >= 5000 and day <= 18:
                orders.append(["BUY_LAND", "SE"])
                cur_money -= 4000
                unlocked_quads.add("SE")
                num_quads += 1

        # Livestock: 6 Geese in NW, ONLY when wheat feed is available (Day 2+ or shed wheat >= 4)!
        target_geese = 6
        total_geese = sum(1 for p in LIVESTOCK_TILES if isinstance(get_tile(*p), dict) and get_tile(*p).get("animal") == "GOOSE") + shed.get("GOOSE", 0) + sum(inv.get("GOOSE", 0) for inv in inventories)

        has_feed_or_early_wheat = (day >= 3 or shed.get("WHEAT", 0) >= 3)
        if has_feed_or_early_wheat and day <= 15 and len(orders) < max_orders:
            while total_geese < target_geese and cur_money >= 400 and len(orders) < max_orders:
                orders.append(["BUY_ANIMAL", "GOOSE", 1])
                cur_money -= 300
                total_geese += 1

        # Seed Purchasing
        empty_tiles = [(x, y) for (x, y) in unlocked_coords if get_tile(x, y) is None and (x, y) not in LIVESTOCK_TILES]
        ...
        if day <= 28 and len(orders) < max_orders:
            ...
            if day in (27, 28):
                ...
                needed_seeds["WHEAT"] = max(needed_seeds["WHEAT"], total_emp + 10)
            else:
                for pos in empty_tiles + weed_tiles:
                    c = get_target_crop_pos(pos, day, remaining_days)
        ...
        for product in PRODUCTS:
            ...
            if product == "WHEAT" and day < 29:
                if qty > 8:
                    orders.append(["SELL", "WHEAT", min(qty - 8, 25)])
                continue
        ...
        for w_idx, w_pos in enumerate(workers):
            ...
            # --- A. Farmer Special: Coops & Geese Pickup/Placement & Feed Pickup ---
            if w_idx == 0 and day <= 28:
                has_anim_inv = inv_w.get("GOOSE", 0) > 0
                has_anim_shed = shed.get("GOOSE", 0) > 0
                needs_feed_count = sum(1 for p in LIVESTOCK_TILES if isinstance(get_tile(*p), dict) and get_tile(*p).get("animal") and not get_tile(*p).get("fed_today", False))

                if not has_anim_inv and has_anim_shed:
                    if w_pos == (4, 4):
                        action = ["PICKUP", "GOOSE", min(has_anim_shed, 2)]
                    else:
            ...
            # --- B. Goose Care on Standing Tile ---
            if action is None and w_pos in LIVESTOCK_TILES and w_pos not in assigned_targets:
            ...
            if action is None:
                if w_pos in harvestable and w_pos not in assigned_targets:
                    ...
                    pref_c = get_target_crop_pos(w_pos, day, remaining_days)
                    chosen_c = None
            ...
            if action is None:
                ...
                if not best_target and w_idx == 0:
                    for g_pos in LIVESTOCK_TILES:
                        if g_pos not in assigned_targets and g_pos not in fed_today:
                            gt = get_tile(*g_pos)
                            if isinstance(gt, dict) and gt.get("animal"):
                                if (not gt.get("fed_today", False) and shed.get("WHEAT", 0) > 0) or gt.get("yield_units", 0) > 0 or gt.get("fertilizer_available", False):
                                    best_target = g_pos
                                    break
                ...
                # High priority weed clearing so weeds never spread
