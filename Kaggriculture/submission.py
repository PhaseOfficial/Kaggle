"""
Master Industrial & ML Opponent-Aware Farm Agent for Kaggriculture.
Built upon the high-yield foundation of commit 00459b210cb4ec91153fe8a80c0cfaa34d6373c3:
- Online Opponent State & Harvest Forecaster: Predicts opponent crop dumps
- Cournot-Nash Best-Response Optimizer: Dynamic market front-running & crop pivoting
- Fibonacci Dynamic Workforce Allocation
- Livestock Hub in NW Plot 1 (Cows & Sheep)
- Zero midnight shed discard loss
"""
...
def agent(obs: dict) -> dict:
    """Standalone Kaggle decision function with ML Opponent Modeling."""
    global _global_tracker
    try:
        state = FarmState(obs)
        opp_id = 1 - state.player_id
        if _global_tracker is None or _global_tracker.opp_id != opp_id:
            _global_tracker = OpponentTracker(opp_id)

        _global_tracker.update(obs)
        planner = MLAdaptiveMacroPlanner(state, _global_tracker)
